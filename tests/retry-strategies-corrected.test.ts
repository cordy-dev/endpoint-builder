...
describe("Retry Strategies", () => {
	...
	describe("JitteredExponentialBackoffRetryStrategy", () => {
		...
		test("should respect max delay with jitter", () => {
			const strategy = new JitteredExponentialBackoffRetryStrategy(5, 1000, 2500, 2, 0.2);
			const error = createHttpError(500);

			// Jitter is applied after maxDelay, so final delay can exceed maxDelay by jitter amount
			// For attempt 3: exponentialDelay = 8000, bounded to 2500, jitter = ±500, so range is [2000, 3000]
	...
	describe("LinearBackoffRetryStrategy", () => {
		...
		test("should use custom increment", () => {
			const strategy = new LinearBackoffRetryStrategy(3, 500, 750);
			const error = createHttpError(500);

			expect(strategy.calculateDelay(0, error)).toBe(500);   // 500 + (0 * 750)
			expect(strategy.calculateDelay(1, error)).toBe(1250);  // 500 + (1 * 750)
			expect(strategy.calculateDelay(2, error)).toBe(2000);  // 500 + (2 * 750)
		});
